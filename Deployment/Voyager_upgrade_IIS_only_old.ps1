...
param([string]$environment="none",[string]$buildversion="none",$Optional,$ngatsharein,$CustomBuildPath,[DateTime]$StartTime)
...
$log_voyager_appLogs="$logroot\voyager" 					# This path is used to make sure the folder is created. Log4Net requires this format when setting value in web.config (d:\\logroot\\Voyager\\logfile)
$scriptexecutedfrom	=$pwd									# Capturing the current path for future consumption
...
Function ModifyConfigFiles ($webconfig, $FeedbackConfigFile, $voyagerUIconfig) {
	...
	##186 - Setting App Fabric Cache name
	if(-not($doc.SelectSingleNode("/configuration/system.web/sessionState/providers/add[@name='AppFabricCacheSessionStoreProvider']")))
	{
		LogMessage "warn" "WARNING!!! App fabric Cache hosts not defined in this web.config"
		LogMessage "warn" "WARNING!!! This indicates possible script failure or change to web.config - Please investigate"}
	else
	{
	...
	$n | where {$_.bindingConfiguration -like "lodgingSupplyBinding" -or `
		...
		foreach {
			...
			$_.address = $_.address -replace '\w*\:.*?\:[0-9]{4,}', $ServiceEndpointURL
		}


	
$TrainingHTTPSExclusionEnvironment = "TRN-DEV","Perf"
## START temporary wrapper to excluse training HTTPS switch (for John beaver since training does not support HTTPS)

if ($environment -notmatch $TrainingHTTPSExclusionEnvironment){
...
if($CustomBuildPath -ne $null -and $CustomBuildPath -match "\\"){
	Logmessage "Info" "Setting Build path to custom path = $optional"
	$buildpath = $CustomBuildPath
	}
...
$folders = get-childitem $SourceDir | ? {$_.PSIsContainer} | Where {$_.name -ne "EndCall" -and $_.name -ne "Feedback"} 
