...
set errorlog= c:\winnt\temp\bfserr.tmp
@REM for /f "eol= tokens=1,2* delims= " %%i in (\\dnfil01\ops\Scripts\Hotfix\sherpa\%2_ bfs_queryserver.txt) do set servername=%%i
...
choice  /tn,15 "%username%, this will update BFS %2 servers. Are you sure you want to do this?"> \\expcpfs01\siteops\aaronb\bfs_out.txt

	for /f %%i in (\\dnfil01\ops\Scripts\Hotfix\sherpa\%2_ sherpa_travelserver.txt) do start psexec \\%%i -u expeso\%username% -p %1 c:\bin\hotfix\travelserver.cmd %datetime% %username% %1
	  
	Goto ErrCkservices
cls
Echo %username%, %2  travelserver servers will be finished soon.  
...
:ErrCkservices
	for /f %%a in (\\dnfil01\ops\Scripts\Hotfix\sherpa\%2_ sherpa_travelserver.txt) do sclist \\%%a|grep -i mstravob > %errorlog%
