...
@REM for /f "eol= tokens=1,2* delims= " %%i in (\\dnfil01\ops\Scripts\Hotfix\sherpa\%2_ sherpa_bots.txt) do set servername=%%i
...
choice  /tn,15 "%username%, this will update Sherpa %2 servers. Are you sure you want to do this?">\\dnrdc02\c$\markus\sherpa_out.txt
	...
	for /f %%i in (\\dnfil01\ops\Scripts\Hotfix\sherpa\%2_ sherpa_bots.txt) do start psexec \\%%i -u expeso\%username% -p %1 c:\bin\hotfix\bots.cmd %datetime% %username% %1
	  
	Goto ErrCkservices
cls
Echo %username%, %2  bots servers will be finished soon.  
...
:ErrCkservices
	for /f %%a in (\\dnfil01\ops\Scripts\Hotfix\sherpa\%2_ sherpa_bots.txt) do sclist \\%%a|grep -i migration > %errorlog%
