...
choice  /tn,15 "%username%, this will update Sherpa, odd followed by even. Are you sure you want to do this?">\\dnrdc02\c$\markus\sherpa_out.txt
	...
	for %%i in (1) do start psexec \\dnbottpa0%%i -u expeso\%username% -p %1 c:\bin\hotfix\TPA_bot.cmd %datetime% %username% %1
	for %%i in (1) do start psexec \\dnfdstpa0%%i -u expeso\%username% -p %1 c:\bin\hotfix\fds.cmd %datetime% %username% %1
	Goto ErrCkOdd
cls
Echo %username%, Odd servers will be finished soon.  
...
Pause
cls
	for %%i in (2 4) do start psexec \\dnwbmtpa0%%i -u expeso\%username% -p %1 c:\bin\hotfix\webs.cmd %datetime% %username% %1
	for %%i in (2) do start psexec \\dnwbotpa0%%i -u expeso\%username% -p %1 c:\bin\hotfix\ops.cmd %datetime% %username% %1
	for %%i in (2) do start psexec \\dnwbotpa0%%i -u expeso\%username% -p %1 c:\bin\hotfix\ops.cmd %datetime% %username% %1
	for %%i in (2 4) do start psexec \\dntvhtpa0%%i -u expeso\%username% -p %1 c:\bin\hotfix\TPA_travel.cmd %datetime% %username% %1
	for %%i in (2) do start psexec \\dnbottpa0%%i -u expeso\%username% -p %1 c:\bin\hotfix\TPA_bot.cmd %datetime% %username% %1
	for %%i in (2) do start psexec \\dnfdstpa0%%i -u expeso\%username% -p %1 c:\bin\hotfix\fds.cmd %datetime% %username% %1
	goto ErrCkEven
:syntax
cls
echo Syntax:
