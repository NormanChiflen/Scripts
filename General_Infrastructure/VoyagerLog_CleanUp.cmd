...
for %%i in (%SERVER_LIST%) do (
	...
	if exist \\%%i\logroot\voyager (
		...
		for /f "skip=30" %%j in ('dir /s /a:-d /b /o:-d d:\logroot\voyager\%%i\*.log') do del %%j >>%LOGFILE%
	)
	...
	if exist \\%%i\c$\windows\system32\winevt\logs (
		...
		for /f "skip=30" %%j in ('dir /s /a:-d /b /o:-d d:\logroot\ApplicationEvent\%%i\app*.evtx') do del %%j >>%LOGFILE%
	)  
	...
	if exist \\%%i\c$\inetpub\logs\LogFiles\ (
		...
		for /f "skip=30" %%j in ('dir /s /a:-d /b /o:-d d:\inetpub\logs\LogFiles\%%i\*.log') do del %%j >>%LOGFILE%
	)  
)
