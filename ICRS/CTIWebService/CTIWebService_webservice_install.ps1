...
param([string]$environment="none",[string]$buildversion="none",$webroot="d:\webroot",$Optional, [DateTime]$StartTime = "1/1/1990")
...
#variables:
$WebSiteName		= "icrscids.expedia.com"
$WebAppName			= "CTIWebService"
...
switch ($DnsDomain)
{
	...
	"cct.expecn.com"
	{
		if ($ServerName.Substring(0,3) -eq "che") {
			...
			$buildstorage		= "\\che-filidx.idx.expedmz.com\Release\sait\CRM\modules\" + $WebAppName + "\Service"
		}elseif($ServerName.Substring(0,3) -eq "phe"){
			# Production Phoenix Jenkins file share
			$buildstorage		= "\\phe-filidxprd.idx.expedmz.com\Release\sait\CRM\modules\" + $WebAppName + "\Service"
		}else{
	...
	"karmalab.net"
	{
		$buildstorage		= "\\karmalab.net\builds\directedbuilds\sait\CRM\modules\" + $WebAppName + "\Service"
		$versionTxt			="version.txt"
		$historyTxt			="History.txt"
	}
...
Function ModifyConfigFiles ($webconfig) {
	...
	$doc.SelectSingleNode("//add[@name='ICRSConfigDSN']").connectionString = $ICRSConfigConnectionString
...
if($Optional -eq "auto"){
	...
	[string]$CurrentVersion=(gc $WebAppFilePath\$versionTxt) -match "\b[v]\d{1}\."
...
BackupWebSiteConfiguration "$WebSiteName" "$webroot_archive\$WebAppName\$WebAppName.$installTimeStamp"
...
# Step ## - Remove and Create Application directory - set apppool etc..
#if (test-path "IIS:\Sites\$WebSiteName\$WebAppName") { ri "IIS:\Sites\$WebSiteName\$WebAppName" -recurse -force }
#if (!(test-path "IIS:\Sites\$WebSiteName\$WebAppName")) {CreateSiteVdir $WebRoot $WebSiteName $WebAppName $WebVDirName $AppPoolUser $AppPoolUserPassword  }
...
$DefaultCSV[$arrayposition]										| out-file "$WebAppFilePath\$versionTxt" -append
...
amail "cctrel@expedia.com" $("mcraig@expedia.com"; "$env:username@expedia.com") "$WebAppName - Deployment on $environment environment - $env:computername finished successfully"  $emailbody -attachment $attachment
