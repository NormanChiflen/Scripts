...
Sub SaveNetworkAdapterSettings ( oAdapter, AdapterIndex, SaveWithMACAddress )
	...
	If isnull(oAdapter.WINSPrimaryServer) and isnull(oAdapter.WINSSecondaryServer) then
...
Function LoadNetworkAdapterSettings ( oAdapter, AdapterIndex )
	...
	If LoadAdapterSetting( AdapterIndex, "IpAddressList" ) <> "" and LoadAdapterSetting( AdapterIndex, "SubnetMask" ) <> ""  then
		...
		CheckForErrors iResult, sErrorString,"EnableStatic(IPAddress,SubnetMask)"
	...
	If LoadAdapterSetting( AdapterIndex, "Gateways" ) <> "" and sCostMetric <> "" then
		...
		CheckForErrors iResult, sErrorString,"SetGateways(Gateway, GatewayMetric)"
	...
	If Item <> "" then
		Select case(Item)
			...
			case else
				oLogging.CreateEntry "OSDAdapter" & AdapterIndex & "TCPIPNetBiosOptions value unknown: [" & Item & "]", LogTypeWarning
		End Select
