...
Class WebService
	...
	Public Function Query
		...
		If UCase(sMethod) = "POST" then
			oLogging.CreateEntry "About to execute web service call using method " & sMethod & " to " & sURL & ": " & sEnvelope, LogTypeInfo
			oHTTP.open "POST", sURL, False, sUserID, sPassword
			oHTTP.setRequestHeader "Content-Type", "application/x-www-form-urlencoded"
			On Error Resume Next
			oHTTP.send sEnvelope
		Else
			oLogging.CreateEntry "About to execute web service call using method " & sMethod & " to " & sURL, LogTypeInfo
