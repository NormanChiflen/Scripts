...
Function CheckForDisk
	...
	If Property("DefaultDestinationIsDirty") <> "" then
		' If we display the wizard once, we should keep on displaying until reset by SetTaskSequenceProperties()
		oLogging.CreateEntry "DefaultDestinationIsDirty = " & Property("DefaultDestinationIsDirty"), LogTypeInfo 
		CheckForDisk = False
		Exit Function
	End if
	...
	oLogging.CreateEntry "CheckForDisk: [" & sDestDisk & "]  Partition: [" & sDestPart & "]", LogTypeInfo

	Set oDisks = objWMI.ExecQuery("Select index from Win32_DiskDrive where MediaType like 'Fixed%hard disk%'")
	
	If not HasGoodDestDisk( sDestDisk ) then
		oLogging.CreateEntry "Could not find disk", LogTypeInfo
		oProperties.Add "DefaultDestinationIsDirty", "BadDisk"
		CheckforDisk = False
		Exit Function
	End if
	
	If not HasGoodDestPart ( sDestDisk, sDestPart ) then
		oLogging.CreateEntry "Target Drive does not match System Drive [" & oEnv("SystemDrive") & "]", LogTypeInfo
		oProperties.Add "DefaultDestinationIsDirty", "BadPart"
		CheckForDisk = False
		Exit Function
	End if 
