...
:: Servers to be un-shunted are AU Primary IVR Web Application servers only

SET WebAppServers=AU_IVR_WEBS.txt

:: Choice Dialog

choice /t:n,10 Are you sure you want to un-shunt AU Primary IVR Node
if errorlevel 2 goto :EOF

:: Overwriting config.xml with Un-Shunted - config.xml

for /f %%i in (\\%SrvListLoc%\%WebAppServers%) do (
attrib -R \\%%i\d$\wwwroot\Advocate\config\config.xml
copy /y \\%%i\d$\wwwroot\Advocate\config\unshunt\config.xml \\%%i\d$\wwwroot\Advocate\config\config.xml
attrib +R \\%%i\d$\wwwroot\Advocate\config\config.xml
)
...
echo AU Primary IVR Node Un-Shunt completed >> \\%StartDir%\message.txt

echo. >> \\%StartDir%\message.txt

echo Un-Shunt applied to the following servers: >> \\%StartDir%\message.txt
