...
param([string]$environment="none",[string]$buildversion="none",$Optional,$webroot="c:\inetpub\adfs\ls",[DateTime]$StartTime = "1/1/1990")
...
If($DnsDomain -match "cct.expecn.com"){
	# CCT Production Settings: Both Chandler and Phoenix can access this share
		$buildstorage		= "\\chefilidx.idx.expedmz.com\Release\sait\CRM\products\$appname"
	...
	#$buildstorage		= "\\karmalab.net\builds\directedbuilds\sait\CRM\products\$appname"			# Test

	$buildstorage		= "\\chclappadfs02\d$\binroot\$appname"
	$versionTxt			="version.txt"
	$historyTxt			="History.txt"

	# $buildstorage		= "\\karmalab.net\builds\directedbuilds\sait\CRM\products\$appname"
...
Function ModifyConfigFiles ($webconfig) {
	...
	LogMessage "info" "Setting ProvisioningEndpoint"
	
	$doc.SelectSingleNode("//client/endpoint").address = $ProvisioningEndpoint
	
	$doc.SelectSingleNode("//add[@key='ICRSRealmText']").value = $ICRSRealmText
...
Foreach($i in $DefaultCSV){
	...
	if ($i.environment -eq $environment){
			...
			$ProvisioningEndpoint = $i.ProvisioningEndpoint
			$ICRSRealmText = $i.ICRSRealmText
...
if($Optional -eq "WebConfigOnly"){
	...
	copy $buildpath\deliverables\ls\web.config      $WebConfigModifiedFile
...
if($webroot -ne "c:\inetpub\adfs\ls"){$multisitepath=$webroot.replace("c:\","");Stop-Website $multisitepath
	}else{
...
# STEP x  - Renaming current folder to folder.Old

if(test-path $webroot){	
	robocopy $webroot $webroot_backup\$appName.$installTimeStamp\ls *.* /E /NP /NS /NFL /NDL
...
$VersionBack = "$webroot_backup\$appName.$installTimeStamp\ls\$VersionTxt"
$oldweb_config= "$webroot_backup\$appName.$installTimeStamp\ls\web.config"
...
$SourceDir= "$buildpath\deliverables\ls"
$DestDir= "$webroot"
#new way of copying
...
$VersionBackSharePath   = "\\$env:computername" +  ".$env:USERDNSDOMAIN" + "\webroot_backup\$appName.$installTimeStamp\ls\$VersionTxt"
$oldweb_configSharePath = "\\$env:computername" +  ".$env:USERDNSDOMAIN" + "\webroot_backup\$appName.$installTimeStamp\ls\web.config"
